...
Environment variables are defined in two files, both using the same variable
names (`NILE_TEST_KEY1/2`, node URLs, contract addresses):

- `integration_test/test.env` — committed template with throwaway keys; the
  reference for examples (`source integration_test/test.env` before running
  them) and read tests.
- `cmd/setup_nile_testnet/test.env` — GENERATED by the setup tool and
  gitignored. The write tests load this file automatically via `loadEnv`, so
  sourcing it first is still useful when invoking them directly.
...
- Writes discovered addresses back into `cmd/setup_nile_testnet/test.env` (it does not modify the committed `integration_test/test.env`).
...
All integration tests are behind the `integration` build tag and skip (not
fail) when their env vars are absent, so plain `go test ./...` never touches
a live network.

Mainnet read-only tests:
```bash
MAINNET_NODE_URL=grpc://grpc.trongrid.io:50051 \
go test -tags integration -v ./integration_test/read_tests/...
```

Nile write tests (state-changing):
```bash
source ./cmd/setup_nile_testnet/test.env
go test -tags integration -v ./integration_test/write_tests/...
```

Notes:
- Tests skip automatically when `NILE_NODE_URL` / key env vars are missing.
- `cmd/setup_nile_testnet/test.env` is GENERATED by the setup tool and is
  gitignored — never commit it; keys live only in local env files.
